...
```bash
curl -fsSL https://raw.githubusercontent.com/eskim2001/dsh-cloud/v0.1.1/scripts/install.sh | sudo bash
```
...
```bash
curl -fsSL "https://raw.githubusercontent.com/eskim2001/dsh-cloud/v0.1.1/scripts/install.sh" | sha256sum
```

```bash
git show v0.1.1:scripts/install.sh | sha256sum
```

**Upgrade** (keeps data and secrets):

```bash
curl -fsSL "https://raw.githubusercontent.com/eskim2001/dsh-cloud/v0.1.2/scripts/install.sh" | sudo bash -s -- update --version 0.1.2
```

**Uninstall** (keeps the database volume and storage pool; add `--purge` to delete data irrecoverably):

```bash
curl -fsSL "https://raw.githubusercontent.com/eskim2001/dsh-cloud/v0.1.1/scripts/install.sh" | sudo bash -s -- uninstall
```
