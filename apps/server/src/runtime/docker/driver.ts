 ...
 * - `/data` 是一块 **Docker 命名卷**，删除容器不删卷 → 实例重建不丢数据。
 *
 ...
 *
 * ⚠️ 一处**没做到**，写在 `createStorage` 上：Docker 命名卷**没有硬容量**。
 */
export class DockerDriver implements RuntimeDriver {
  ...
  /**
   * 建数据卷。同名已存在 → `StorageExistsError`（D18「绝不静默覆盖」）。
   *
   * ⚠️ **硬容量没做到。** 契约要求 `sizeMb` 是硬上限（灌满即 ENOSPC），而 Docker
   * 命名卷**没有配额**：它只是 `/var/lib/docker/volumes/<name>` 下的一个目录，共享宿主
   * 文件系统。真要做到只能靠宿主文件系统的配额（XFS project quota 之类），那是**宿主
   * 侧、且仅 Linux** 的事 —— 见 docs/RUNTIME-CONTAINER-EVAL.md。
   *
   * 所以这里的处理是：**把声明的容量记进卷的 label**，让上层能展示、让 `copyStorage`
   * 能按同样容量重建；配额本身留给宿主侧实现。**不是"忘了做"**，是这一层做不到。
   */
  ...
  /**
   * 已用容量（MiB）。**停机时也可读** —— 卷是宿主上的一块目录，不依赖容器在跑。
   *
   * 实现：起一个一次性容器 `du -sm`。为什么不直接读宿主的路径：卷的挂载点在
   * Docker 的虚拟机文件系统里（macOS/Windows），宿主根本看不到；跑个容器是唯一
   * 跨平台的办法。
   */
   ...
   *
   * 做法比 microVM 那版干净：microVM 要去猜上游的私有磁盘布局（`volumes/<name>/disk.raw`），
   * 这里两卷同时挂进一个一次性容器，`cp -a` 就完事 —— 不需要知道卷在宿主的哪儿。
   *
   * **前提：源卷已经没有容器在写**（调用方 `provisioner.setImage` 保证先停了）。
   */
  async copyStorage(fromKey: string, toKey: string): Promise<void> {
    ...
    await this.createStorage(toKey, sizeMb)
  ...
  /** 卷上记录的声明容量。没有（老卷 / 手工建的）就给个下限，别把新卷建得比它还小。 */
  ...
  async create(spec: InstanceSpec, ctx: RenderContext): Promise<RenderedInstance> {
    ...
    const container = await this.docker.createContainer({
      ...
      HostConfig: {
        ...
        Binds: r.mounts.map((m) => `${m.storageKey}:${m.guest}:${m.mode}`),
   ...
   *
   * 用途：`du`（量卷）和 `cp`（复制卷）—— 两件都必须"在卷所在的文件系统上"做的事，
   * 而宿主看不到那个文件系统（Docker 的虚拟机里）。
   */
