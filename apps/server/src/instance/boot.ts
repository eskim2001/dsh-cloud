...
export interface BootDeps {
  ...
  /**
   * 确认该实例的**宿主数据目录**存在且可用。**不允许新建**——目录没了就该报错。
   */
 ...
 *
 * ② **校验所有实例的宿主数据目录**。microVM 时代不再有「挂载」这回事
 *    （数据目录就是宿主上的普通目录，VM 内的副本由 `:staged` 负责），
 *    但**这条检查必须留着**：目录不见了就标 error 并跳过——宁可让实例显式坏掉，
 *    也不能让它带着一个空目录起来。那看起来像「数据没了」，比报错糟得多。
 *
