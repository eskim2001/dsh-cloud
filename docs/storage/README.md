...
释放留**墓碑**而不是删记录：`xfs_quota` 的账是**按 project ID** 记的，不是按目录，只要还有
文件带着那个 projid，用量就还算在它头上；把 id 发给下一个租户就等于把上一个租户的残留
算进新租户的账（代码见 `ProjectRecord.released`）。

对账（`ProjectRegistry.reconcile`）把"注册表里有、盘上已经没"的条目标成墓碑 ——
它的作用不是回收 id，而是别让 `get()` 把已经不存在的存储报成"有效配额"。
⚠️ **它目前没有接进启动流程**（只有单测在跑它），所以出过手删目录的情况要靠它收拾时，
得手动调用或者先接上。
